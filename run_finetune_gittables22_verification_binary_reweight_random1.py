...
for pos_ratio in [0.2]:
    for lr in [1e-4]:
        for task in ['gt-semtab22-dbpedia-all0']: # 'gt-semtab22-dbpedia-all0'
            comment = "Random1-mode@{}-context@{}-data@{}-lr@{}-warmup@{}-dp@{}-norm@{}-pos@{}".format(veri_module, context, version, lr, warmup_ratio, dropout_prob, norm,  pos_ratio)
            cmd = '''CUDA_VISIBLE_DEVICES={} python supcl_ft_verifier_binary_random1.py --wandb True \
