...
if __name__ == "__main__":
    ...
    if args.from_scratch:
        if "gt" in task:
            tag_name = "{}/{}-{}-unlabeled{}-bs{}-ml{}-ne{}-do{}{}".format(
                taskname,  "{}-fromscratch".format(shortcut_name), args.small_tag, args.max_unlabeled,
        ...
        if "gt" in task:
            tag_name = "{}/{}_{}-unlabeled{}-bs{}-ml{}-ne{}-do{}{}".format(
                taskname, args.cl_tag.replace('/', '-'),  shortcut_name, args.small_tag, args.max_unlabeled,
                batch_size, max_length, num_train_epochs, args.dropout_prob,
                '-rs{}'.format(args.random_seed) if args.random_seed != 4649 else '')
        else:
            ...
            if "popl" in task:
                ...
                if "sato" in task or "gt-" in task:
                    if 'gt-' in task and '-all' in task: # TODO
                        ...
                        ts_logits_list += new_filtered_logits.cpu().detach().numpy().tolist()
                    else:
                        ts_pred_list += filtered_logits.argmax(
                            1).cpu().detach().numpy().tolist()
                        ts_true_list += batch["label"].cpu().detach().numpy(
                        ).tolist()
                        ts_logits_list += filtered_logits.cpu().detach().numpy().tolist()
                elif "turl" in task:
