...
if __name__ == "__main__":
    ...
    for epoch in range(num_train_epochs):
        ...
        for batch_idx, batch in enumerate(veri_dataloader):
            ...
            pos_scores, neg_scores = scores[:, 0].reshape(len(scores), -1),  scores[:, 1:].reshape(len(scores), -1)
            scores = torch.cat([pos_scores, neg_scores], dim=-1)
            loss = loss_fn(scores, torch.zeros(len(scores), dtype=torch.long).to(device))
