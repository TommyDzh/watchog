...
if __name__ == "__main__":
    ...
    valid_table = wandb.Table(columns=valid_columns)
    ...
    for repeat_i in range(args.repeat):
        ...
        valid_table.add_data(repeat_i, best_vl_micro_f1, best_vl_macro_f1, 
                                best_vl_loss, best_vl_micro_f1s_epoch, best_vl_macro_f1s_epoch,
                                np.mean(time_epochs))
    ...
    if accelerator.is_local_main_process and args.wandb:
        for macro_macro, macro_micro, micro_macro, micro_micro in zip(ts_macro_f1_all["f1_macro"], ts_macro_f1_all["f1_micro"], ts_micro_f1_all["f1_macro"], ts_micro_f1_all["f1_micro"]):
            test_table.add_data(macro_macro, macro_micro, micro_macro, micro_micro)
        for f1_name in ["f1_macro", "f1_micro"]:
            wandb.log({
                f"test/{f1_name}:micro_f1": np.mean(ts_micro_f1_all[f1_name]),
                f"test/{f1_name}:macro_f1": np.mean(ts_macro_f1_all[f1_name]),
                f"test/{f1_name}:micro_f1_std": np.std(ts_micro_f1_all[f1_name]),
                f"test/{f1_name}:macro_f1_std": np.std(ts_macro_f1_all[f1_name]),
            }, commit=True)
        wandb.log({
            "test/time": np.mean(test_time),
