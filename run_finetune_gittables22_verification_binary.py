...
for lr in [ 5e-4, 1e-3, 5e-3, 1e-2]:
    for task in ['gt-semtab22-dbpedia-all0']:
        comment = "Binary-lr@{}-pos@{}".format(lr, pos_ratio)
        cmd = '''CUDA_VISIBLE_DEVICES={} python supcl_ft_verifier_binary.py --wandb True \
                    --shortcut_name {} --task {} --pos_ratio {} --use_attention_mask True --max_length {} --lr {} --max_unlabeled {} --batch_size {} --epoch {} \
                    --dropout_prob {} --pretrained_ckpt_path "{}" --cl_tag {} --small_tag "{}" --comment "{}" {} {} {}'''.format(
            gpus, base_model, task,  pos_ratio, ml, lr, max_unlabeled, bs, n_epochs, dropout_prob,
