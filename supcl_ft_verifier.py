...
from watchog.dataset import GittablesTablewiseIterateDataset, VerificationDataset
...
if __name__ == "__main__":
    ...
    with accelerator.main_process_first():
        if task in [
            ...
            if task[-1] in "01234":
                cv = int(task[-1])
                veri_dataset = VerificationDataset(max_list_length=args.max_list_length)
    ...
    verifier = Verifier(module=args.veri_module).to(device)
    ...
    for epoch in range(num_train_epochs):
        ...
        for batch_idx, batch in enumerate(veri_dataloader):
            ...
            num_samples += len(scores)//args.max_list_length
            loss = listMLE(scores.reshape(-1, args.max_list_length), batch["label"].to(device).reshape(-1, args.max_list_length))
        ...
        if (epoch+1) % args.eval_interval == 0 or epoch > num_train_epochs//2:
            ...
            with accelerator.main_process_first():
                ...
                print(
                    "Epoch {} ({}): tr_loss={:.7f} ({:.2f} sec.)"
                    .format(epoch, task, tr_loss, time_epoch_train),
                ...
                if ts_macro_f1 > best_vl_macro_f1:
                    ...
                    torch.save(best_state_dict_macro, "{}_verifier_best_f1_macro.pt".format(file_path))
                if ts_micro_f1 > best_vl_micro_f1:
                    ...
                    torch.save(best_state_dict_micro, "{}_verifier_best_f1_micro.pt".format(file_path))
    if accelerator.is_local_main_process and args.wandb:
        wandb.finish()
    # with accelerator.main_process_first():
    #     if args.eval_test:
