...
for context in ["target"]:
    for lr in [1e-4, 1e-3]:
        for task in ['gt-semtab22-dbpedia-all0']: # 'gt-semtab22-dbpedia-all0'
            comment = "mode@{}-context@{}-lr@{}-warmup@{}-dp@{}-norm@{}-pos@{}".format(veri_module, context, lr, warmup_ratio, dropout_prob, norm, pos_ratio)
            cmd = '''CUDA_VISIBLE_DEVICES={} python supcl_ft_verifier_binary.py --wandb True \
                        --shortcut_name {} --warmup_ratio {} --norm {} --veri_module {} --context {} --reweight True --task {} --pos_ratio {} --use_attention_mask True --max_length {} --lr {} --max_unlabeled {} --batch_size {} --epoch {} \
