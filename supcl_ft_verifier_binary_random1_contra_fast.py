...
from watchog.dataset import SOTABTablewiseIterateDataset, GittablesTablewiseIterateDataset, VerificationBinaryFastDataset, GittablesTablewiseIterateClusterDataset
        ...
        if task in [
            ...
            if task[-1] in "01234":
                ...
                if args.data_version is not None or args.data_version != "None":
                    veri_dataset = VerificationBinaryFastDataset(data_path=f"/data/zhihao/TU/Watchog/verification/{args.task}/random1_veri_data_{args.data_version}.pth", pos_ratio=args.pos_ratio if not args.reweight else None, context=args.context)
                else:
                    veri_dataset = VerificationBinaryFastDataset(data_path=f"/data/zhihao/TU/Watchog/verification/{args.task}/random1_veri_data.pth", pos_ratio=args.pos_ratio if not args.reweight else None, context=args.context)
