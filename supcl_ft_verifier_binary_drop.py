...
from watchog.dataset import SOTABTablewiseIterateDataset, GittablesTablewiseIterateDataset, VerificationBinaryDropDataset
...
if __name__ == "__main__":
    ...
    parser.add_argument("--context", type=str, default="concat") 
    parser.add_argument("--data_version", type=str, default="0") 
    parser.add_argument("--label_version", type=int, default=0) 
        ...
        if task in [
            ...
            if task[-1] in "01234":
                ...
                if args.data_version is not None or args.data_version != "None":
                    veri_dataset = VerificationBinaryDropDataset(data_path=f"/data/zhihao/TU/Watchog/verification/{args.task}_veri_drop_data_{args.data_version}.pth", 
                                                                 context=args.context,
                                                                 label_version=args.label_version)
                else:
                    veri_dataset = VerificationBinaryDropDataset(data_path=f"/data/zhihao/TU/Watchog/verification/{args.task}_veri_drop_data.pth", 
                                                                 context=args.context,
                                                                 label_version=args.label_version)
                ...
                if args.test_version is None or args.test_version == "None":
                    test_dataset = torch.load(f"/data/zhihao/TU/Watchog/verification/{args.task}_test_data_drop.pth")
                else:
                    test_dataset = torch.load(f"/data/zhihao/TU/Watchog/verification/{args.task}_test_data_drop_{args.test_version}.pth")
    ...
    if args.context == "concat":
        input_size = 2
    elif args.context == "sbert":
        input_size = 3
    else:
        input_size = 1
    verifier = Verifier(module=args.veri_module, dropout=args.dropout_prob, norm=args.norm, input_size=input_size).to(device)
    ...
    best_vl_micro_f1_drop1 = -1
    best_vl_macro_f1_drop1 = -1
    best_vl_loss = 1e10
    ...
    for epoch in range(num_train_epochs):
        ...
        for batch_idx, batch in enumerate(veri_dataloader):
            ...
            if args.loss == "MSE":
                ...
                loss = (loss * pos_weights).mean()
            else:
                loss = loss_fn(scores.squeeze(), batch["label"].to(device).squeeze().float())
        ...
        if (epoch+1) % args.eval_interval == 0 or epoch > num_train_epochs//2:
            ...
            with accelerator.main_process_first():
                ...
                with torch.no_grad():
                    for batch_idx, batch in enumerate(test_dataloader_iter):
                        ...
                        if len(init_permutation_i) ==1:
                            ...
                            logits_test_topk.append(logits.detach().cpu())
                            continue
                        ...
                        for i in range(1, len(test_embs[batch_idx])):
                            ...
                            if args.context == "concat":
                                embs_temp = torch.cat([embs, embs_temp.reshape(-1)])
                            elif args.context == "substract":
                                embs_temp = embs - embs_temp.reshape(-1)
                            elif args.context == "substract_abs":
                                embs_temp = torch.abs(embs - embs_temp.reshape(-1))
                            elif args.context == "sbert":
                                embs_temp = torch.cat([embs, embs_temp.reshape(-1), torch.abs(embs - embs_temp.reshape(-1))])
                            else:
                                raise ValueError("context {} is not supported".format(args.context))
                        ...
                        cols_to_drop = init_permutation_i[init_permutation_i!=0][torch.tensor(col_scores[batch_idx]).argsort(descending=True)][:num_cols_to_drop].tolist()
                        ...
                        logits_test_topk.append(logits_topk.detach().cpu())
                    labels_test = torch.cat(labels_test, dim=0)
                    logits_test = torch.stack(logits_test, dim=0)
                    logits_test_topk = torch.stack(logits_test_topk, dim=0)
                    preds_test = torch.argmax(logits_test, dim=1)
                    preds_test_topk = torch.argmax(logits_test_topk, dim=1)
                ...
                ts_micro_f1_drop1 = f1_score(labels_test.reshape(-1).numpy().tolist(),
                                    ts_pred_list,
                                    average="micro")
                ts_macro_f1_drop1 = f1_score(labels_test.reshape(-1).numpy().tolist(),
                                    ts_pred_list,
                                    average="macro")
                
                ts_pred_list_topk = logits_test_topk.argmax(
                                            1).cpu().detach().numpy().tolist()
                ts_micro_f1_drop1_topk = f1_score(labels_test.reshape(-1).numpy().tolist(),
                                    ts_pred_list_topk,
                                    average="micro")
                ts_macro_f1_drop1_topk = f1_score(labels_test.reshape(-1).numpy().tolist(),
                                    ts_pred_list_topk,
                                    average="macro")
                ...
                # if ts_macro_f1_drop1 > best_vl_macro_f1_drop1:
                #     best_vl_macro_f1_drop1 = ts_macro_f1_drop1
                #     best_vl_macro_f1s_epoch = epoch
                #     best_state_dict_macro = deepcopy(verifier.state_dict())
                #     torch.save(best_state_dict_macro, "{}_verifier_binary_best_f1_macro_drop1.pt".format(file_path))
                # if ts_micro_f1_drop1 > best_vl_micro_f1_drop1:
                #     best_vl_micro_f1_drop1 = ts_micro_f1_drop1
                #     best_vl_micro_f1s_epoch = epoch
                #     best_state_dict_micro = deepcopy(verifier.state_dict())
                #     torch.save(best_state_dict_micro, "{}_verifier_binary_best_f1_micro_drop1.pt".format(file_path))         
                ...
                print(
                    ...
                    "ts_macro_f1_drop1={:.4f} ts_micro_f1_drop1={:.4f} ({:.2f} sec.)"
                    .format(ts_macro_f1_drop1, ts_micro_f1_drop1, time_epoch_test))
                if accelerator.is_local_main_process and args.wandb:
                    wandb.log({
                            f"train/loss": tr_loss,
                            f"train/time": time_epoch_train,
                            f"train/learning_rate": current_lr,
                            f"test/micro_f1_drop1": ts_micro_f1_drop1,
                            f"test/macro_f1_drop1": ts_macro_f1_drop1,
                            f"test/micro_f1_drop1_topk": ts_micro_f1_drop1_topk,
                            f"test/macro_f1_drop1_topk": ts_macro_f1_drop1_topk,
    ...
    #                                             "ts_macro_f1_drop1", "ts_micro_f1_drop1",
